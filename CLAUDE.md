...
`pyproject.toml` declares the ranges and `uv.lock` pins the resolved set. That is
the only dependency source: the Docker image installs with `uv sync --frozen`, so
the image, CI and a local checkout all get the same versions.

There used to be a second one. `requirements.txt` and `requirements/requirements-test.txt`
were generated by hatch-pip-compile and installed only by the Docker image, and
they drifted badly - the image shipped fastmcp 2.14.7 while `uv.lock` still pinned
2.2.5. It also produced 15 dependabot PRs that all proposed downgrades by the time
anyone read them, and regenerating on macOS silently dropped Linux-only packages
the image needs. Both files are gone; do not reintroduce a second lockfile.

Refresh with `uv lock --upgrade` (or `hatch run update`, which now calls it).
